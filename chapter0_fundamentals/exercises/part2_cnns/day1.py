...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        scale_fac = 2 / np.sqrt(in_features)
        
        self.weight = nn.Parameter(t.Tensor(scale_fac*(t.rand(out_features,in_features) - 0.5)))

        if bias == False:
            self.bias = None
        else:
            self.bias = nn.Parameter(scale_fac*(t.rand(out_features) - 0.5))
        #.  W x
        #dim of W is out_features*in_features
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        x = t.einsum('...i,...ji->...j', [x, self.weight])
...
def train(args: SimpleMLPTrainingArgs) -> tuple[list[float], list[float],SimpleMLP]:
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, labels in mnist_testloader:
            ...
            correct_cnt += (t.argmax(logits, dim=-1) == labels).sum().item()
        accuracy_list.append(correct_cnt/len(mnist_testset))
...
class Conv2d(nn.Module):
    def __init__(
        ...
        self.weight = nn.Parameter(2 * sf * (t.rand(out_channels, in_channels, kernel_size, kernel_size) - 0.5))
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        shaper = lambda x: einops.rearrange(x, "j -> () j () ()")
        y = (x - shaper(mean)) / t.sqrt(shaper(var) + self.eps)
        y = y * shaper(self.weight) + shaper(self.bias)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.left = Sequential(Conv2d(in_feats,out_feats,3,stride=first_stride,padding=1),
        ...
        if is_shape_preserving:
            ...
            self.right = Sequential(Conv2d(in_feats, out_feats, 1,stride=first_stride, padding=0),
            BatchNorm2d(out_feats))
...
class BlockGroup(nn.Module):
    def __init__(self, n_blocks: int, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.blockgroup = Sequential(ResidualBlock(in_feats, out_feats, first_stride), *[ResidualBlock(out_feats, out_feats) for _ in range(n_blocks - 1)])
...
class ResNet34(nn.Module):
    def __init__(
        ...
        self.starting_layers = Sequential(nn.Conv2d(3, out_feats0, kernel_size=7, stride=2, padding=3),
        nn.BatchNorm2d(out_feats0),
        nn.ReLU(),
        nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        )
        
        features_per_group_temp = [64, *self.out_features_per_group]

        print([64, *self.out_features_per_group])
        print([out_feats0] + out_features_per_group)

        self.block_groups = Sequential(*[BlockGroup(n_blocks_per_group[i],features_per_group_temp[i],features_per_group_temp[i+1],first_stride=first_strides_per_group[i]) for i in range(len(n_blocks_per_group))]
        )
        ...
        print(self.block_groups)
        print("hello")
        print(self.residual_layers)

        self.final_layers = Sequential(AveragePool(),Linear(out_features_per_group[-1],1000))
